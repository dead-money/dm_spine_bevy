...
- `cargo run --release --example spine_stress` — stress test. Spawns N skeletons (default 1) in a perfect-square grid centered on the origin, all running spineboy's `run` animation with per-instance time offsets. Press **]** to step up to the next square (1 → 4 → 9 → 16 → …) and **[** to step down. **R** resets, **Esc** quits. HUD reports `fps / tick_ms / build_ms` via Bevy diagnostics. Run with `--release` — debug builds skew the numbers heavily. CSV export: `--csv path.csv` writes `frame,count,fps,tick_ms,build_ms` per frame. On the dev machine (RTX 4090 + i9-14900K) the runtime + Bevy integration sustain ~1500 running spineboys before frame drops at 60 Hz vsync; per-skeleton cost is ~10 µs CPU at scale (tick + mesh-build).
